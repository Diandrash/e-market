...
@section('container')
    <section class="product-details pt-24 mx-4 md:mx-10 pb-24">
        <div class="product-area flex flex-wrap">
            ...
            <div class="product-detail ml-0 md:ml-10">
                ...
                <div class="order-area mt-10 md:mt-40">
                    <form action="/products/{{ $product->id }}/checkout" 
                        method="post">
                        @csrf
                        <input type="hidden" name="product_id" 
                        value="{{ $product->id }}">
                        <label for="quantity" class="font-semibold text-base">
                            Quantity</label>
                        <input type="number" min="1" class="w-12 h-8 font-black 
                        ml-3 pl-5" value="1" name="quantity" id="quantity">
                        @auth
                            <button type="submit" class="w-full h-14 mt-8 
                            bg-emerald-600 font-semibold text-white 
                            text-xl rounded-lg">
                        @endauth
                        @guest
                        <button disabled class="w-full h-14 mt-8 bg-emerald-600
                         font-semibold text-white text-xl rounded-lg opacity-30">
                        @endguest
                            Buy Now
                        </button>
                    </form>
        ...
        <div class="profile-area mt-14">
            <div class="content flex justify-between bg-white rounded-lg shadow-sm
             w-full p-4">
